...
void VideoFrameMng::AddVideoFrame(bool capture, int64_t time, const char* data, int size, int width, int height, const std::string& json, FrameType frame_type)
{
	...
	//{
	//	uid = valus[nrtc::kNRTCDeviceDataUid].asUInt64();
	//}
	if (!capture && uid == 0)
	{
		return;
	...
	{
		auto it = recv_video_pic_list_.find(uid);
		...
		else
		{
			...
			recv_video_pic_list_[uid] = pic_info;
		}
	}
}
bool VideoFrameMng::GetVideoFrame(uint64_t uid, int64_t& time, char* out_data, int& width, int& height, bool mirror, bool argb_or_yuv)
{
	nbase::NAutoLock auto_lock(&lock_);
	timeb time_now;
	ftime(&time_now); // 秒数
