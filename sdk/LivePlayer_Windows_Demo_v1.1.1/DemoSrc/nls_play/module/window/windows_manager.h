...
public:
	...
	*/
	...
	static WindowType* SingletonShow(const std::wstring& window_id, bool show, bool isLayeredWindow = true)
	{
		WindowType *window = (WindowType*)(WindowsManager::GetInstance()->GetWindow(WindowType::kClassName, window_id));
		if (!window)
		{
			window = new WindowType;
			window->Create(NULL, WindowType::kClassName, WS_OVERLAPPEDWINDOW & ~WS_MAXIMIZEBOX, 0, isLayeredWindow);
			window->CenterWindow();
			if (show)
			{
				window->ShowWindow();
