...
//////////////////////////////////////////////////////
// os
// 从Win8.1开始，GetVersionEx已经不被支持。VerifyVersionInfo似乎也是错误的。

BOOL GetNtVersionNumbers(LPOSVERSIONINFO plpVerInfo)
{
	BOOL bRet = FALSE;
	if (plpVerInfo == NULL)
		return bRet;
	...
	bool call_succeed = true;
	LoadModuleAndCallProcVoid<PfRtlGetNtVersionNumbers>(L"ntdll.dll", "RtlGetNtVersionNumbers", \
		call_succeed, &plpVerInfo->dwMajorVersion, &plpVerInfo->dwMinorVersion, &plpVerInfo->dwBuildNumber);
	if (call_succeed)
	{
		plpVerInfo->dwBuildNumber &= 0x0ffff;
		bRet = TRUE;
	}
	if (!bRet)
		bRet = ::GetVersionEx(plpVerInfo);
...
int GetOSVersion()
{
	...
	if (!GetNtVersionNumbers(&version_info))
		return -1;

	return version_info.dwMajorVersion * 100 + version_info.dwMinorVersion;
}
...
std::string GetOSVersionString()
{
	...
	if (IsWindowsServer())
	{
		...
		if (os_ver == 502)
			return "Windows Server 2003 or 2003 R2";
		return nbase::StringPrintf("Windows Server (OS Version %d)", os_ver);
	}
	else
	{
		if (os_ver == 1000)
		...
		if (os_ver == 502)
			return "Windows XP Professional x64 SP2";
		if (os_ver == 501)
		{
		...
		return nbase::StringPrintf("Windows (OS Version %d)", os_ver);
	}
}
...
{
	BOOL is_wow64 = FALSE;
	BOOL ret = LoadModuleAndCallProc<LPFN_ISWOW64PROCESS>(L"kernel32.dll", "IsWow64Process", FALSE, GetCurrentProcess(), &is_wow64);
	if (!ret)
		return false;
...
std::string GetUUID()
{
	...
	return nbase::BinaryToHexString(&guid, 16);
}
...
std::string GetMD5(const std::string &source)
{
	...
	encrypt_enc->SetMethod(nbase::ENC_MD5);
	std::string md5;
	encrypt_enc->Encrypt(source, md5);
	return nbase::BinaryToHexString(md5);
}
